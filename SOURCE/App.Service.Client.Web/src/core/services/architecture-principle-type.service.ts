...
export class ArchitecturePrincipleTypeService {
  principleTypes = signal<ArchitecturePrincipleTypeViewModel[]>([]);
  loading = signal(false);
  error = signal<string | null>(null);
  
  typeCount = computed(() => this.principleTypes().length);
  hasTypes = computed(() => this.principleTypes().length > 0);
    ...
    return this.repository.getAll().pipe(
      tap({
        next: (dtos) => {
          ...
          this.principleTypes.set(viewModels);
          this.loading.set(false);
          this.logger.debug(`Loaded ${viewModels.length} principle types`);
        },
        error: (err) => {
          this.error.set('Failed to load principle types');
          this.loading.set(false);
          this.logger.error(`Error loading principle types: ${err?.message || err}`);
        }
      })
    ).subscribe();
  }
  ...
  getById(id: string): ArchitecturePrincipleTypeViewModel | undefined {
    return this.principleTypes().find(type => type.id === id);
  }
