...
export class AboutService {
  ...
  private _theme = signal<ThemeInfoDto | null>(null);
  ...
  hasThemeInfo = computed(() => this._theme() !== null);

  // Data file paths - use service-level data (shared across accounts)
  ...
  /**
   * Load theme info from JSON
   */
  private loadThemeInfo(): Observable<ThemeInfoDto | null> {
    return this.http.get<ThemeInfoDto>(`${this.dataPath}/theme.json`).pipe(
      catchError(() => of(null))
    );
  }
